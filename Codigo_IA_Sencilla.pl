...
introducirCol(X,[' '],[X]):- !.
introducirCol(X,[' ',E|C2],[X,E|C2]):- E \== (' '), !.
introducirCol(X,[' '|C2],[' '|CA2]):- introducirCol(X,C2,CA2).
...
%Se comprueba si el contricante haganado la partida
jugarMaquinaAleatorio(X):- victoria('X',X),
    nl,write('X GANA!').
jugarMaquinaAleatorio(X):- empate(X),
    nl,write('EMPATE').
jugarMaquinaAleatorio(X):- repeat,
                           random_between(0,7,N),
                           introducir('O',N,X,X2), !,
                           mostrarTablero(X2),
                           jugando('X',X2).
%función aleatoria para que la máquina eliga posición